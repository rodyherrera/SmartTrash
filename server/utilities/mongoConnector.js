...
const mongoConnector = async () => {
    ...
    console.log(`[CleverBin Cloud Server]: Connecting to MongoDB (${databaseName})...`);
    const uri = process.env.MONGO_URI + '/' + databaseName;
    mongoose.set('strictQuery', false);
    mongoose.set('strictPopulate', false);
    try{
        await mongoose.connect(uri, { authSource: 'admin' });
        console.log('[CleverBin Cloud Server]: Connected to MongoDB!');
    }catch(error){
        console.log('[CleverBin Cloud Server]: An unhandled error has been ocurred while trying to connect to MongoDB.');
        console.log(error);
    }
};
